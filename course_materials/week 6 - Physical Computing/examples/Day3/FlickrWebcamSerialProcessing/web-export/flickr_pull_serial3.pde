...
int DISPLAY_TIME = 100; // 2000 ms = 2 seconds
...
void setup() {
  ...
  String query = api + "tags=endangered&format=rest&extras=url_o&api_key=562b5c8df6a49c0f5db3562b7e0de4c4&per_page=+500);";
  XML xml;
  xml = loadXML(query);
...
void draw() {
  ...
  int rate = int(map(sensorValue, 70, 200, 50, 1000));
  int opac = int(map(sensorValue, 70, 200, 1, 255));
  int range1 = int(map(sensorValue, 50, 130, 1, 255));
  int range2 = int(map(sensorValue, 50, 130, 255, 1));
